      ...
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {categories.map((c) => (
          <a
            key={c.id}
            href="#produtos"
            className="group relative overflow-hidden rounded-2xl border border-border bg-card p-8 shadow-card-luxe transition-all hover:-translate-y-1 hover:shadow-luxe"
          >
            <div className="absolute inset-0 bg-gradient-luxe opacity-0 transition-opacity duration-500 group-hover:opacity-10" />
            <span className="relative font-display text-5xl text-gradient-luxe">{c.icon}</span>
            <h3 className="relative mt-4 font-display text-xl">{c.name}</h3>
            <p className="relative mt-1 text-sm text-muted-foreground">Ver coleção →</p>
          </a>
        ))}
      </div>
    </section>
